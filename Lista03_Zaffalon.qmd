...
```
**Primeira query:** Mostra todas as 6 linhas da tabela na ordem em que foram salvas, sem nenhum tipo de filtro ou ordenação.

**Segunda query:** O DISTINCT avalia as duas colunas juntas; como não existe nenhum funcionário com o mesmo departamento e o mesmo salário ao mesmo tempo, nenhuma linha é repetida e o resultado continua trazendo as 6 linhas.

**Terceira query:** Pega os mesmos resultados únicos e os organiza em ordem crescente pelo departamento e, em caso de empate no departamento, desempatando pelo menor salário.
...
```
**Resposta:** Elas são lógicamente equivalentes, pois C1 AND (C2 OR C3) = (C1 AND C2) OR (C1 AND C3).
...
```
**Resposta:** Elas são lógicamente equivalentes, pois C1 OR (C2 AND C3) = (C1 OR C2) AND (C1 OR C3).
...
```
**Resposta:** Somente a primeira e a terceira cláusulas WHERE são equivalentes, pois NOT (C1 AND C2) = (NOT C1) OR (NOT C2). Para a segunda ser equivalente ela deveria ser WHERE NOT ACRES < 50 OR STATE <> 'MA'.
...
WHERE NOT ACRES < 50 AND NOT STATE = 'MA'
```
**Resposta:** Somente a primeira e a terceira cláusulas WHERE são equivalentes, pois NOT (C1 OR C2) = (NOT C1) AND (NOT C2). Para a segunda ser equivalente ela deveria ser igual a terceira.
