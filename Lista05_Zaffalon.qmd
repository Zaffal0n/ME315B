...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES + ACRES * (20.0/100.0) AS ACRES_AUMENTO
FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES/3.0 AS ACRES_REDUZIDO
FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE + 50) / 2.0 AS FEE_AJUSTADA
FROM PRESERVE
```
...
COUNT(DISTINCT PNAME) AS TOTAL_NOMES_DIF
FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(ACRES * 0.02) AS MEDIA_NOVA_TAXA_AZ
FROM PRESERVE
WHERE STATE = 'AZ'
```
...
SALARY + SALARY * (10.0/100.0) AS NEWSALARY
FROM EMPLOYEE
ORDER BY NEWSALARY
```
